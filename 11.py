...
class EEGMonitorWindow(QMainWindow):
    ...
    def push_remote_eeg(self):
        ...
        raw_hist = list(self.parser.raw_wave_history)
        if self.last_raw_push_len > len(raw_hist):
            self.last_raw_push_len = 0
        raw_values = raw_hist[self.last_raw_push_len:]
        self.last_raw_push_len = len(raw_hist)
        payload = {
            "device_id": DEVICE_ID,
            "attention": int(self.parser.attention),
            "meditation": int(self.parser.meditation),
            "poor_signal": int(self.parser.poor_signal),
            "raw_value": raw_hist[-1] if raw_hist else None,
            "raw_values": raw_values[-512:],
