...
REMOTE_EEG_URL = f"http://{B_HOST}:{B_PORT}/eeg/latest"
REMOTE_EEG_WAVE_URL = f"http://{B_HOST}:{B_PORT}/eeg/wave"
DEVICE_ID = "a_host_eeg_01"
...
class EEGMonitorWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        self.push_session = requests.Session()
        self.push_session.trust_env = False
    ...
    def _update_remote_latest(self):
        ...
        try:
            resp = self.remote_pull_session.get(REMOTE_EEG_URL, timeout=0.4)
            payload = resp.json() if resp.ok else {}
            wave_resp = self.remote_pull_session.get(REMOTE_EEG_WAVE_URL, timeout=0.4)
            if wave_resp.ok:
                payload["raw_values"] = wave_resp.json().get("raw_values")
        except Exception:
            return False

        att = payload.get("attention")
        med = payload.get("meditation")
        if att is None and med is None:
            return False
        try:
            if att is not None:
                self.parser.attention = max(0, min(100, int(att)))
            if med is not None:
                self.parser.meditation = max(0, min(100, int(med)))
            poor = payload.get("poor_signal")
            if poor is not None:
                self.parser.poor_signal = max(0, min(200, int(poor)))
            raw = payload.get("raw_value")
            raw_values = payload.get("raw_values")
            if isinstance(raw_values, list) and raw_values:
                self.parser.raw_wave_history.clear()
                self.parser.raw_wave_history.extend(
                    int(v) for v in raw_values[-15000:] if v is not None)
            elif raw is not None:
                self.parser.raw_wave_history.append(int(raw))
            eeg_power = payload.get("eeg_power")
            if isinstance(eeg_power, dict):
                self.parser.eeg_power.update({
                    k: int(v) for k, v in eeg_power.items()
                    if k in self.parser.eeg_power and v is not None
                })
        except Exception:
            return False

        self.mode = self.MODE_SERIAL
        self.parser.last_update = now
    ...
    def update_display(self):
        ...
        if self.mode == self.MODE_SERIAL:
            self.push_remote_eeg()

    def toggle_raw_data(self, state):
        self.show_raw_data = (state == Qt.Checked)

    def push_remote_eeg(self):
        if self.mode != self.MODE_SERIAL:
            return
        now = time.time()
        if now - self.last_push_ts < PUSH_INTERVAL_SEC:
            return
        ...
        try:
            self.push_session.post(POST_URL, json=payload, timeout=1.5)
